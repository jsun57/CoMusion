...
class ClassifierForFID(nn.Module):
    def __init__(self, input_size=48, hidden_size=128, hidden_layer=2, output_size=15, use_noise=None, device="", dtype=torch.float32):
    ...
    def forward(self, motion_sequence, hidden_unit=None):
        ...
        gru_o, _ = self.recurrent(motion_sequence.to(self.dtype), hidden_unit) # [100, b, 48]
        # dim (num_samples, 30)
    ...
    def get_fid_features(self, motion_sequence, hidden_unit=None):
        ...
        gru_o, _ = self.recurrent(motion_sequence.to(self.dtype), hidden_unit)
        # dim (num_samples, 30)
